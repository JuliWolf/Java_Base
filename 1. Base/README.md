  ...
  + [2.1. Основы](#21-основы)
    ...
    + [2.10. Multithreading](#210-multithreading)
      <details>
        ...
        + [2.10.18. Блокирующие очереди](#21018-блокирующие-очереди)
...
+ [4. База данных](#4-база-данных)
  ...
  + [4.1. Основы баз данных и PostgreSQL](#41-основы-баз-данных-и-postgresql)
    ...
    + [4.1.13. Тип ключа `unique`](#4113-тип-ключа-unique)
...
### 2.9.1. Задачи classloader
ClassLoader отвечает за динамическую загрузку классов в JVM. Его основные задачи включают:
- Поиск и загрузку байт-кода классов из различных источников (файлы .class, JAR-архивы, сети, базы данных и т.д.).
- Создание экземпляров класса `Class` на основе загруженного байт-кода.
- Обеспечение изоляции классов: классы, загруженные разными ClassLoader'ами, считаются разными, даже если имеют одинаковые имена.
- Поддержка ленивой загрузки: классы загружаются только при первом обращении к ним.
- Управление жизненным циклом классов, включая их выгрузку (хотя полная выгрузка возможна только при выгрузке самого ClassLoader'а).

Это позволяет Java-приложениям быть динамичными и модульными, поддерживая плагины, веб-приложения и другие сценарии, где классы загружаются во время выполнения.

### 2.9.2. Этапы получения работающего JVM кода
Процесс загрузки и подготовки класса к использованию включает три основных этапа: загрузку (Loading), связывание (Linking) и инициализацию (Initialization). Эти этапы выполняются последовательно и только при первом обращении к классу.

- **Загрузка (Loading)**:
    - Поиск байт-кода класса по его полному имени (например, `com.example.MyClass`).
    - Проверка, не был ли класс уже загружен ранее (в кеше ClassLoader'а).
    - Если не загружен, получение байт-кода из источников (файлы, JAR, сеть и т.д.).
    - Проверка корректности байт-кода (например, магическое число `CAFEBABE`).
    - Создание экземпляра `Class<?>` и размещение его в кеше.
    - Рекурсивная загрузка родительских классов и интерфейсов, если они еще не загружены.

- **Связывание (Linking)**:
    - **Verification**: Проверка корректности байт-кода на соответствие спецификации JVM (например, отсутствие недопустимых инструкций, правильность типов, отсутствие переполнения стека). Это предотвращает выполнение вредоносного кода.
    - **Preparation**: Выделение памяти для статических полей класса и инициализация их значениями по умолчанию (0 для чисел, null для объектов, false для boolean).
    - **Resolution**: Разрешение символьных ссылок на другие классы, поля, методы и константы. Это может происходить лениво (во время выполнения), а не обязательно на этом этапе.

- **Инициализация (Initialization)**:
    - Выполнение статических инициализаторов (статических блоков и инициализации статических полей).
    - Вызов статического конструктора `<clinit>`.
    - Этот этап происходит только один раз для каждого класса и синхронизирован, чтобы избежать проблем в многопоточной среде.

Эти этапы обеспечивают, что класс полностью готов к использованию, включая создание экземпляров и вызов методов.

### 2.9.3. Условия для выполнения этапов
- Класс должен быть полностью загружен (Loading) перед переходом к связыванию (Linking).
- Связывание должно завершиться (Verification, Preparation и, опционально, Resolution) перед инициализацией (Initialization).
- Ошибки на этапе Resolution могут возникать не только во время Linking, но и во время выполнения программы (например, при первом обращении к методу или полю). Это называется ленивым разрешением (lazy resolution).
- Инициализация триггерится при первом активном использовании класса (например, создание экземпляра, вызов статического метода или поля), а не при пассивной ссылке (например, объявление переменной типа этого класса).
- В многопоточной среде инициализация синхронизирована, чтобы предотвратить двойную инициализацию.

Эти правила гарантируют корректность и безопасность загрузки классов.

### 2.9.4. Типы загрузчиков
В Java используется иерархическая модель ClassLoader'ов, где каждый загрузчик имеет родителя. Иерархия выглядит так:
- **Bootstrap ClassLoader** (корневой, также называется Primordial ClassLoader): Загружает базовые классы JDK (из `rt.jar`, `jmods` в Java 9+). Реализован нативно в JVM, без Java-кода.
- **Extension ClassLoader** (Platform ClassLoader в Java 9+): Загружает классы расширений из директории `jre/lib/ext` (или `java.ext.dirs`). Наследуется от Bootstrap и загружает платформенно-специфичные классы.
- **System ClassLoader** (Application ClassLoader): Загружает классы приложения из CLASSPATH (переменная окружения или `-cp`). Это загрузчик, используемый по умолчанию для main-класса.

Дополнительно, приложения могут создавать собственные ClassLoader'ы (например, URLClassLoader для загрузки из сети или WebAppClassLoader в серверах приложений).

Иерархия обеспечивает делегирование: запрос на загрузку передается вверх по цепочке, что позволяет базовым классам быть загруженными первыми и предотвращает конфликты.

### 2.9.5. От какого класса наследуются все загрузчики?
Все пользовательские ClassLoader'ы наследуются от абстрактного класса `java.lang.ClassLoader`. Этот класс предоставляет базовую функциональность, такую как методы для загрузки классов, делегирования родителям и кеширования.

Bootstrap ClassLoader является исключением — он реализован нативно и не наследуется от `ClassLoader`.

### 2.9.6. От какого класса наследуется базовый загрузчик?
Bootstrap ClassLoader не наследуется от `java.lang.ClassLoader` — он является частью JVM и реализован на нативном коде (C/C++). Он не имеет родителя и служит корнем иерархии.

### 2.9.7. Какие методы надо реализовывать при реализации Classloader
При создании собственного ClassLoader обычно переопределяют следующие методы из `java.lang.ClassLoader`:
- `loadClass(String name)` или `loadClass(String name, boolean resolve)`: Основной метод для загрузки класса с возможностью разрешения ссылок.
- `findLoadedClass(String name)`: Проверяет, загружен ли класс уже в кеше.
- `findClass(String name)`: Реализует логику поиска и загрузки байт-кода из специфических источников (например, файлов, сети). Это ключевой метод для кастомной логики.
- `defineClass(String name, byte[] b, int off, int len)`: Преобразует байт-код в экземпляр `Class` (обычно вызывается из `findClass`).
- `resolveClass(Class<?> c)`: Выполняет Resolution, если требуется.
- `getParent()`: Возвращает родительский ClassLoader (для делегирования).

Не все методы нужно переопределять — базовая реализация в `ClassLoader` предоставляет шаблон делегирования.

### 2.9.8. Какой метод является точкой входа для загрузки классов
Точкой входа является метод `loadClass(String name)`, который инициирует процесс загрузки. Он проверяет кеш, делегирует родителю и, если необходимо, вызывает `findClass` для загрузки.

### 2.9.9. Логика работы classLoader
Логика основана на принципе делегирования:
1. Вызывается `loadClass(String name)` (или `loadClass(String name, boolean resolve)`).
2. Проверяется кеш: `findLoadedClass(String name)` — если класс уже загружен, возвращается его `Class`.
3. Если не найден, запрос делегируется родительскому ClassLoader'у (рекурсивно до Bootstrap).
4. Если родитель не нашел класс, вызывается `findClass(String name)` для поиска байт-кода в собственных источниках.
5. После загрузки байт-кода он верифицируется, и создается `Class` с помощью `defineClass`.
6. Если `resolve` == true, выполняется `resolveClass` для разрешения ссылок.
7. Класс кешируется и возвращается.

Это обеспечивает уникальность и безопасность: классы загружаются ближайшим к корню загрузчиком, способным их найти.

### 2.9.10. Принципы загрузки классов
- **Делегирование (Delegation)**: Запрос передается родителю; загрузка происходит только если родитель не справился. Это обеспечивает, что базовые классы загружаются Bootstrap'ом, а не переопределяются.
- **Видимость (Visibility)**: ClassLoader видит только свои классы и классы предков; потомки не видят классы друг друга. Это изолирует модули.
- **Уникальность (Uniqueness)**: Класс загружается только однажды в рамках своего ClassLoader'а. Два ClassLoader'а могут загрузить одинаковый класс как разные типы.
- **Ленивость (Laziness)**: Классы загружаются по требованию, что оптимизирует память и время запуска.

Эти принципы предотвращают конфликты версий и обеспечивают безопасность.

### 2.9.11. Схема загрузки классов
Схема представляет поток загрузки с делегированием:

1. Программа запрашивает класс `{name}`.
2. System ClassLoader проверяет кеш: если найден, возвращает `Class`.
3. Если нет, делегирует Extension ClassLoader.
4. Extension проверяет кеш; если нет, делегирует Bootstrap.
5. Bootstrap проверяет кеш; если нет, пытается загрузить из rt.jar.
6. Если Bootstrap не нашел, возвращает управление Extension, который пытается загрузить из ext/.
7. Если Extension не нашел, возвращает System, который вызывает `findClass` для поиска в CLASSPATH.
8. Если ни один не нашел, выбрасывается `ClassNotFoundException`.
...
### 2.9.12. Все ли классы загружаются сразу
Нет, классы загружаются лениво (lazy loading) — только при первом обращении. Это включает три этапа:
1. **Loading**: Загрузка байт-кода и создание `Class` при первом использовании.
2. **Linking**: Verification, Preparation, Resolution — Resolution может быть отложено.
3. **Initialization**: Выполнение статических инициализаторов при первом активном использовании (создание экземпляра, вызов статического метода и т.д.).

Это оптимизирует ресурсы: например, в большом приложении не все классы нужны сразу.

### 2.9.13. Как создать собственный ClassLoader
Для создания кастомного ClassLoader:
1. Наследуйтесь от `java.lang.ClassLoader`.
2. Переопределите `findClass(String name)`, чтобы реализовать поиск байт-кода (например, из базы данных или сети).
3. Используйте `defineClass` для создания `Class` из байт-кода.
4. Опционально переопределите `loadClass` для кастомной логики делегирования.
...
public class CustomClassLoader extends ClassLoader {
    ...
    protected Class<?> findClass(String name) throws ClassNotFoundException {
        // Логика поиска байт-кода, например, из файла
        byte[] bytes = loadClassData(name);
        return defineClass(name, bytes, 0, bytes.length);
    }
}
```
Это полезно для плагинов, динамической загрузки или изоляции классов.

### 2.9.14. Что такое ClassNotFoundException и когда оно возникает
`ClassNotFoundException` — это checked exception, выбрасываемое, когда ClassLoader не может найти или загрузить класс по имени. Возникает в:
- `Class.forName(String name)` или `ClassLoader.loadClass(String name)`, если класс не найден в источниках.
- При разрешении ссылок, если зависимый класс отсутствует.

Причинами могут быть: неправильный CLASSPATH, отсутствие JAR, ошибки в имени класса или проблемы с сетью (для удаленных загрузчиков).

### 2.9.15. Разница между ClassLoader и Class.forName()
- `ClassLoader.loadClass(String name)`: Загружает класс без инициализации (если не указан resolve=true). Возвращает `Class<?>`, но не вызывает статические инициализаторы.
- `Class.forName(String name)`: Загружает и инициализирует класс, вызывая статические блоки. Использует текущий ClassLoader контекста (обычно System).

`Class.forName` удобен для явной загрузки с инициализацией (например, JDBC-драйверы), в то время как `loadClass` — для ленивой загрузки.

### 2.9.16. Как ClassLoader влияет на безопасность в Java
ClassLoader обеспечивает песочницу (sandboxing):
- Изоляция: Классы из разных загрузчиков не могут взаимодействовать напрямую, предотвращая конфликты и атаки.
- Verification: Проверяет байт-код на безопасность (например, отсутствие вредоносных инструкций).
- Делегирование: Гарантирует, что системные классы загружаются доверенным Bootstrap'ом, предотвращая подмену (например, подмена `String`).
- В Java Security Manager (устарел) ClassLoader контролировал доступ к ресурсам.

В современных Java (с модулями) это усиливается инкапсуляцией модулей.

### 2.9.17. Что такое модульная система в Java 9 и как она связана с ClassLoader
Модульная система (JPMS, Java Platform Module System) введена в Java 9 для лучшей инкапсуляции и управления зависимостями.
- Модули группируют классы и ресурсы; каждый модуль имеет descriptor с зависимостями.
- ClassLoader'ы адаптированы: Platform ClassLoader загружает модульные классы.
- Связи: Модули влияют на видимость классов — классы в модуле доступны только если экспортированы. ClassLoader'ы работают в рамках модулей, обеспечивая stricter isolation.
- Пример: Bootstrap загружает core modules (java.base), а кастомные ClassLoader'ы могут работать с unnamed modules для legacy-кода.

Это делает ClassLoader более безопасным и предсказуемым.
...
### 2.10.1. В чем разница между sleep() и wait()?
wait - освобождает монитор</br>
sleep- не освобождает монитор

### 2.10.2. От какого количества начинается многопоточность?
- от двух

### 2.10.3. Какие есть издержки у многопоточности?
...
### 2.10.4. Какие потоки создает JVM при запуске?
- Главный поток main
- Потоки для служебных задач (например сбора мусора)

### 2.10.5. Какой класс является потокобезопасным?
...
### 2.10.6. Какие операции являются атомарными
...
### 2.10.7. Что может являться монитором?
...
### 2.10.8. Как обезопасить использование объектов
...
### 2.10.9. Как создать потокобезопасный класс
...
### 2.10.10. Что происходит при использовании `synchronized`
- Попытка блокирования монитора
- Блокировка потока
- Разблокировка монитора
- Разблокировка потока

### 2.10.11. Как работает неблокирующая read блокировка
...
### 2.10.12. Как работает Оптимистическая блокировка
...
### 2.10.13. Как работают классы Atomic из пакета `java.util.concurrent.atomic`
- Использует "Оптимистическую блокировку"

### 2.10.14. Чем отличается Runnable от Callable
| Runnable                                                                                 | Callable                                                                         |
|------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------|
...
### 2.10.15. Конкурентные коллекции - классы для работы с коллекциями внутри потоков
- `ConcurrentHashMap`
...
### 2.10.19. Паттерн "Производитель-потребитель"
...
### 2.10.21. Блокирующие и прерываемые методы
...
### 2.10.22. Синхронизаторы - Классы для создания синхронизирующих точек
...
### 2.10.23. Синхронизатор - Защелки - latch
...
### 2.10.26. `ReentrantReadWriteLock`
...
### 2.10.27. Синхронизаторы - FutureTask
...
### 2.10.28. Синхронизаторы - Семафоры
...
### 2.10.30. Синхронизаторы - Барьеры
...
### 2.10.31. `CyclicBarrier`
- Выполняет синхронизацию заданного количества потоков в одной точке

### 2.10.32. Какие недостатки у неограниченного количества потоков
...
### 2.10.34. Методы Executors
- `newFixedThreadPool` — фиксированный пул потоков
- `newCachedThreadPool` — кэшированный пул с динамическим размером
- `newSingleThreadExecutor` — один поток для последовательной обработки
- `newScheduledThreadPool` — пул с поддержкой отложенных и периодических задач

### 2.10.35. Методы жизненного цикла Executors
```java
public interface ExecutorService extends Executor {
    void shutdown();
    List<Runnable> shutdownNow(); // Возвращает задачи, не начатые к выполнению
    boolean isShutdown();
...
### 2.10.36. Что такое "утечка потока"
- Поток не может поймать непроверяемое исключение, выброшенное из задачи TimerTask, что приводит к завершению потока

### 2.10.37. Жизненный цикл задачи, выполняемой исполнителем
- Создана
...
### 2.10.38. Что такое CompletionService
- Комбинирует функционал `Executor` и блокирующей очереди `BlockingQueue`
- Позволяет отправлять задачи `Callable` и получать готовые результаты через методы `take` и `poll`

### 2.10.39. ExecutorCompletionService
- Использует один `Executor` для выполнения задач
- Хранит количество задач и подсчитывает завершённые результаты
- Позволяет узнать, когда все результаты готовы

### 2.10.40. Сколько куч будет при множественных потоках
- Куча общая для всех потоков, доступ к ней имеют все потоки

### 2.10.41. Что такое shutdown hook
Shutdown hook — это поток, регистрируемый в JVM, который выполняется при её выключении. Используется для:
...
+ [4.1. Основы баз данных и PostgreSQL]()
    <details>
        <summary>Навигация</summary>
...
+ [4.4. Пулы соединений и дополнительные темы]()
  <details>
      <summary>Навигация</summary>
...
#### 4.1.1. Что такое схема, таблица
- **Схема** - это формальное описание структуры и взаимосвязи таблиц и других объектов в БД. Включает:
    - Определение таблиц.
...
- **Таблица** - основной элемент базы данных, состоящий из:
    - Столбцов (columns) с определенными типами данных.
    - Строк (rows), содержащих фактические данные.

Для **junior**: Схема как чертеж дома, где таблица - комната, столбцы - мебель, строки - вещи в мебели.

Для **senior**: В PostgreSQL схема - это namespace для объектов БД, позволяющий группировать таблицы, views и т.д. для изоляции и управления правами доступа (например, `CREATE SCHEMA myschema;`).

#### 4.1.2. В чем разница между Statement и PreparedStatement?
- **Statement** - не защищен от SQL-инъекций, так как запросы формируются динамически без предварительной подготовки.
- **PreparedStatement** - переданные параметры защищены от SQL-инъекций благодаря предварительной компиляции запроса и привязке параметров (placeholders).

Для **junior**: Statement как открытый замок - легко взломать; PreparedStatement как замок с ключом.

Для **senior**: PreparedStatement использует плейсхолдеры (?) для безопасной передачи данных, улучшает производительность при повторных запросах за счет кеширования плана выполнения. В JDBC: `PreparedStatement ps = conn.prepareStatement("SELECT * FROM users WHERE id = ?"); ps.setInt(1, id);`.

#### 4.1.3. План исполнения запроса
Пример в PostgreSQL с командой `EXPLAIN`.  
`EXPLAIN [(OPTION [,...])]` анализирует запрос без выполнения (по умолчанию) или с выполнением.
- **Analyze [boolean]** - запрос выполнится, и составится план выполнения с реальными затратами.
- **Verbose [boolean]** - добавит дополнительную информацию о выводе (например, имена столбцов).
- **Costs [boolean]** - добавит стоимость каждого узла плана (startup cost и total cost).
- **Buffer [boolean]** - определит число блоков, прочитанных с диска и из кеша.
- **Timing [boolean]** - время запуска и выполнения на каждом узле.
- **Summary [boolean]** - общая сводка.
- **Format [TEXT|XML|JSON|YAML]** - формат вывода для отчетов.

Пример: `EXPLAIN (ANALYZE, VERBOSE, COSTS, BUFFERS, TIMING) SELECT * FROM users WHERE age > 30;`.

Для **junior**: Как карта маршрута - показывает, как БД пройдет от старта к результату, с временем и стоимостью.

Для **senior**: План помогает оптимизировать запросы; например, если индекс не используется, план покажет Seq Scan вместо Index Scan. В PostgreSQL pg_stat_statements для мониторинга планов.

#### 4.1.4. Функция vs Процедура
**Функции**
- Имеют возвращаемый тип и значение (RETURN).
- Внутри невозможны DML (INSERT, UPDATE, DELETE); только SELECT.
- Нет выходных аргументов (OUT).
- Невозможен вызов процедуры из функции.
- Можно вызывать в SELECT-запросах.
...
**Процедуры**
- Нет возвращаемого типа, но есть выходные аргументы (OUT/INOUT).
- Возможны DML-запросы.
- Есть входные и выходные аргументы.
- Можно управлять транзакциями (COMMIT/ROLLBACK внутри).
- Невозможно вызывать в SELECT.

Пример создания:

```sql
CREATE OR REPLACE PROCEDURE update_user(user_id INT, new_name TEXT)  
LANGUAGE plpgsql  
AS $$  
BEGIN  
...
Для **junior**: Функция как калькулятор (дает результат); процедура как робот (выполняет действия, может менять данные).

Для **senior**: Функции используются в выражениях; процедуры для сложной логики с side-effects. В PostgreSQL процедуры появились в v11, до этого только функции с VOID.

#### 4.1.5. Что означает ACID
- **Atomicity** (Атомарность): Транзакция либо выполнена полностью, либо не выполнена совсем. Нет промежуточных состояний.
- **Consistency** (Согласованность): Транзакция переводит БД из одного согласованного состояния в другое.
- **Isolation** (Изолированность): Параллельные транзакции не влияют друг на друга.
- **Durability** (Надежность): После COMMIT изменения сохранены, даже при сбое.

Примеры:
1. Атомарность: Перевод денег - либо оба счета обновлены, либо ни один.
2. Согласованность: Все обязательные поля заполнены; пользователь и контакты сохранены вместе.
3. Изоляция: Изменения одной транзакции не видны другой до COMMIT.
4. Надежность: После подтверждения COMMIT данные не потеряны.

Для **junior**: ACID как гарантии банка - деньги либо переведены безопасно, либо нет.

Для **senior**: В PostgreSQL реализован через MVCC (Multiversion Concurrency Control) для изоляции; WAL (Write-Ahead Logging) для durability.

#### 4.1.6. Какие ошибки отменяют транзакцию?
В PostgreSQL транзакция отменяется при ошибках, нарушающих ACID, таких как:
- Ошибки синтаксиса или типов данных (e.g., division by zero).
- Нарушения ограничений (constraints): UNIQUE, FOREIGN KEY, CHECK.
- Deadlocks (взаимные блокировки).
- Ошибки сериализации (serialization failures) на уровне SERIALIZABLE.
- Системные ошибки (e.g., out of memory).

Для **junior**: Ошибки как красный свет - транзакция стопорится и откатывается.

Для **senior**: Не все ошибки отменяют (e.g., NOTICE не отменяет); SAVEPOINT позволяет частичный откат.

#### 4.1.7. Как удалить все записи из таблицы
- Из одной таблицы:
    - TRUNCATE table_name; - быстрый, не логируется, сбрасывает автоинкремент.
    - DELETE FROM table_name; - логируется, медленнее, но можно с WHERE.
- Из нескольких таблиц: TRUNCATE table_a, table_b;.

Для **junior**: TRUNCATE как стирание доски; DELETE как удаление по одной записи.

Для **senior**: TRUNCATE не вызывает триггеры, быстрее для больших таблиц; DELETE можно откатить (если не COMMIT).

#### 4.1.8. Как работает while loop
В PostgreSQL (PL/pgSQL) цикл WHILE выполняется, пока условие истинно.
...
Для **junior**: Как счетчик - повторяет действия, пока условие не ложно.

Для **senior**: Избегать в проде из-за производительности; использовать для динамической логики, но предпочитать FOR или курсоры.

#### 4.1.9. Что такое postgres view
View - виртуальная таблица, хранящая результат SELECT-запроса. Предоставляет быстрый доступ к часто используемым данным.

Для **junior**: View как ярлык на папку - показывает данные без хранения.

Для **senior**: Materialized views хранят данные физически для скорости; обычные views пересчитываются каждый раз.

#### 4.1.10. Какие бывают типы ключей
- check constraints.
...
Для **junior**: Правила для данных - как замки и ключи.

Для **senior**: В PostgreSQL constraints обеспечивают целостность; exclusion для сложных условий (e.g., диапазоны).

#### 4.1.11. Тип ключа `check`
Определяет булево выражение для столбца.

Для **junior**: Проверка как фильтр - не пропускает плохие данные.

Для **senior**: Выполняется при INSERT/UPDATE; можно добавить позже: ALTER TABLE ADD CONSTRAINT.

#### 4.1.12. Тип ключа `not-null`
Столбец не может быть NULL.

Для **junior**: Обязательное поле - как имя на форме.

Для **senior**: NULL отличается от пустой строки; для автоинкремента используйте SERIAL.

#### 4.1.13. Тип ключа `unique`
Значения уникальны (NULL считается уникальным).

Для **junior**: Никаких дубликатов - как уникальный ID.

Для **senior**: Автоматически создает индекс; для FK часто комбинируют с NOT NULL.

#### 4.1.14. Тип ключа `primary keys`
Уникальный идентификатор (UNIQUE + NOT NULL).

Для **junior**: Главный ключ - как паспорт.

Для **senior**: Только один PK на таблицу; часто автоинкрементный.

#### 4.1.15. Тип ключа `foreign keys`
Ссылка на столбец другой таблицы.

Для **junior**: Связь как ниточка между таблицами.

Для **senior**: Обеспечивает referential integrity; CASCADE для автоматического удаления.

#### 4.1.16. Типы поведений при использовании foreign keys
- ON DELETE RESTRICT: Запрет удаления.
- ON DELETE CASCADE: Удалить связанные записи.
- ON DELETE NO ACTION: Ошибка при проверке (дефолт).
- ON DELETE SET NULL: Установить NULL.
- ON DELETE SET DEFAULT: Установить дефолт.

Аналогично для UPDATE.

Для **junior**: Что делать при удалении родителя - как правила семьи.

Для **senior**: CASCADE полезен для зависимостей; SET NULL для опциональных связей.

#### 4.1.17. Тип ключа `exclusion constrains`
Запрещает пересечения по оператору (e.g., && для диапазонов).

Для **junior**: Не пересекайся - как зоны в карте.

Для **senior**: Использует GiST-индекс; для сложных ограничений без UNIQUE.

#### 4.1.18. JSON в PostgreSQL
PostgreSQL поддерживает JSON/JSONB для хранения структурированных данных.
- JSON: Текстовый, сохраняет формат.
- JSONB: Бинарный, оптимизирован для поиска.  
  Операторы: ->, ->>, #> , @>.

Для **junior**: JSON как гибкий контейнер для данных.

Для **senior**: JSONB индексируется (GIN); используйте для NoSQL-подобных данных в реляционной БД.

#### 4.1.19. Массивы и операторы массивов
PostgreSQL поддерживает массивы: INT[], TEXT[].  
Операторы: =, <>, && (пересечение), @> (содержит).

Для **junior**: Массивы как списки в столбце.

Для **senior**: Индексируйте с GIN; полезны для тегов, категорий.

### END ---------------- 4.1. Основы баз данных и PostgreSQL ----------------

### 4.2. Индексы

+ [4.2.1. Как работает индекс в таблице](#421-как-работает-индекс-в-таблице)

#### 4.2.1. Как работает индекс в таблице
1. Без индексов таблица - куча (Heap), сканируется вся.
2. Преимущества: Ускоряет поиск, производительность.
3. Недостатки: Много места, замедляет INSERT/UPDATE/DELETE.
4. Структура: B-деревья (B-tree), сбалансированные. Указатели на строки.
5. Виды:
    - Составной (несколько колонок).
    - Функциональный (по выражению, e.g., UPPER(name)).
    - B-tree (дефолт).
    - Hash (только равенство, медленнее).
    - GiST (гео, полнотекст).
    - SP-GiST (несбалансированные структуры).
    - GIN (инвертированные, для массивов/JSON).
    - BRIN (сводки диапазонов).

Для **junior**: Индекс как оглавление книги - быстро найти страницу.

Для **senior**: Выберите тип по запросам (e.g., GIN для полнотекста); мониторьте с pg_stat_user_indexes.
...
#### 4.3.1. Блокировка таблицы
Виды:
- Разделяемые (shared): Чтение.
- Монолитные (exclusive): Чтение/запись.

Табличные блокировки (LOCK TABLES) - для больших операций, но снижают параллелизм.  
Пользовательские: GET_LOCK для межприложений.  
Глобальные: Для бэкапа.  
Построчные: Лучший контроль, но deadlock-риск.

Для **junior**: Блокировки как очередь - жди своей очереди.

Для **senior**: Используйте минимальный уровень; мониторьте с pg_locks.

#### 4.3.2. Оптимистическая/пессимистическая блокировки
- **Оптимистическая**: Транзакции независимы, проверка конфликтов перед COMMIT.
- **Пессимистическая**: Блокировка при чтении.

В Hibernate: @Version для optimistic.

Для **junior**: Optimistic как "верим в лучшее"; pessimistic как "предупреждаем проблемы".

Для **senior**: Optimistic для низкой конкуренции; pessimistic для высокой.

#### 4.3.3. Транзакции
Совокупность операций, либо все, либо ничего. Основаны на MVCC.  
Свойства ACID.  
Риски: Lost update, dirty read и т.д.  
Уровни изоляции: Read uncommitted, Read committed (дефолт), Repeatable read, Serializable.

Для **junior**: Транзакция как банковская операция - все или ничего.

Для **senior**: MVCC позволяет snapshots; SERIALIZABLE предотвращает anomalies, но медленнее.

#### 4.3.4. Как происходит блокировка во время транзации в Postgresql
Многоуровневая система для согласованности.
- Табличные: Блок всей таблицы.
- Построчные: Конкретные строки.

Процесс: Блокировка при начале, чтении, изменении; освобождение при завершении.  
Гарантии: Изоляция изменений.

Для **junior**: Как светофор - регулирует трафик данных.

Для **senior**: MVCC минимизирует блокировки; deadlocks детектируются автоматически.

#### 4.3.5. Виды блокировок
Табличные: ACCESS SHARE, ROW SHARE, ROW EXCLUSIVE, SHARE UPDATE EXCLUSIVE.  
Построчные: FOR UPDATE, FOR NO KEY UPDATE, FOR SHARE, FOR KEY SHARE.
...
Для **junior**: Разные уровни как скорости интернета.

Для **senior**: Выберите по операции; FOR UPDATE для exclusive access.

#### 4.3.6. Оконные функции в PostgreSQL
Выполняют агрегаты над окном строк. Не группируют.

Функции: ROW_NUMBER(), RANK(), LEAD(), LAG().

Для **junior**: Окно как взгляд через стекло - считает без изменения строк.

Для **senior**: Эффективны для аналитики; PARTITION BY как GROUP BY, но строки сохраняются.

#### 4.3.7. CTE (Common Table Expressions)
WITH для читаемости и рекурсии.

Рекурсивный: Для иерархий.

Для **junior**: CTE как подзапрос с именем.

Для **senior**: Оптимизируется как subquery; материализуется если нужно.

### END ---------------- 4.3. Блокировки и транзакции ----------------
...
#### 4.4.1. Пул соединений к БД
Шаблон для снижения накладных расходов.  
Этапы: Открытие соединения, TCP, чтение/запись, закрытие.  
Пул (e.g., HikariCP) держит открытые соединения.

Для **junior**: Пул как бассейн - готовые соединения.

Для **senior**: Настройте размер пула по нагрузке; мониторьте с JMX.

#### 4.4.2. Репликация в PostgreSQL
Копирование данных на другие серверы.  
Типы: Streaming (logical/physical), Log shipping.

Для **junior**: Реплика как копия диска - для бэкапа и чтения.

Для **senior**: Logical для разнородных систем; physical для высокой доступности.

#### 4.4.3. VACUUM и ANALYZE
- **VACUUM**: Удаляет dead tuples, освобождает место. VACUUM FULL перестраивает таблицу.
- **ANALYZE**: Обновляет статистику для планировщика.

Для **junior**: VACUUM как уборка; ANALYZE как подсчет.

Для **senior**: Авто-VACUUM включен; ручной для больших изменений.

### END ---------------- 4.4. Пулы соединений и дополнительные темы ----------------
