...
+ [10. Как работает кеширование REST](#9-когда-лучше-использовать-rest)
+ [11. Что такое идентификатор ресурса](#9-когда-лучше-использовать-rest)
+ [12. Что такое многоуровневая система](#9-когда-лучше-использовать-rest)
...
- keep-alive - 
  ...
  а после выполнения закрывается.
  Эту проблему пытались решить в версии 1.0 с помощью keep-alive, которое позволяло создавать постоянные соединения
  Для этого клиенту необходимо передать заголово `Connection: keep-alive`
  В версии 1.1 keep-alive стал указываться по умолчанию

3. Пересылка частями (chuncked)
...
2. Мультиплексирование - При передаче информация бьется на небольшие бинарные кусочки - фреймы. Они собираются в потоки, для каждого запрашиваемого ресурса создается отдельный.
...
4. Приоритизация потоков - Позволяет управлять порядком обработки и передачи потоков данных на одном соединении. Улучшает производительность, делает управление ресурсами эффективнее
...
Проблема: Браузеры не могут отличить было ли действие явно совершено или пользователь неумышленно выполнил действие
...
До java 7 string pool находился в отдельной области памяти, который GB не чистил
...
Поверхностное копирование позволяет копировать значения простого объекта в новый объкт, не включая значения объекта
...
Возможность копировать значения составного объекта в другой новый
...
+ [23. Как происходит блокировка во время транзации в Postgresql](#22-тип-ключа-exclusion-constrains)
...
+ [18. Есть ли JPA для mongoDB](#17-реализация-составного-ключа)
