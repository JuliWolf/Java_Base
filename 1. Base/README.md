...
+ [10. Как работает кеширование REST](#10-как-работает-кеширование-rest)
+ [11. Что такое идентификатор ресурса](#11-что-такое-идентификатор-ресурса)
+ [12. Что такое многоуровневая система](#12-что-такое-многоуровневая-система)
...
### 10. Как работает кеширование REST
1. Клиент отправляет запрос на сервер API
2. Сервер API обрабатывает запрос и генерирует ответ
3. Сервер API может добавить заголовки к ответу, которые указывают клиенту, как кэшировать ответ
4. Клиент кеширует ответ в соотвествтии с инструкциями в заголовках
5. Когда клент снова отправляет тот же запрос, он сначала проверяет свой кэш
6. Если ответ найден к кэше, клиент использует его вместо того, чтобы обращаться к серверу API
7. Если ответ не найден в кэше клиент отправляет запрос на сервер 

### 11. Что такое идентификатор ресурса
**Идентификатор ресурса** - это уникальная метка, которая определяет ресурс
**Ресурс** - это абстратное понятие, которое представляет собой любой объект, доступные через API:
- Данные: записи в базе данных, изображения, файлы
- Функциональные возможности: операции, которые можно выполнить над ресурсами, например, содание, обновление или удаление

**Идентификатор ресурса должен быть:**
- Уникальным
- Постоянным
- Стабильным: должен быть неизменным при изменении состояния ресурса
- Читаемым: должен быть понятным для человека и легко читаемым

**Пример:**
- `/users/123` – ИР пользователя с ID 123.
- `/images/cat.jpg` – ИР изображения кота.
- `/products/create` – ИР для создания нового продукта.

**Идентификаторы ресурсов используются для:**
- Доступа к ресурсам: используется в URL-адресах запросов для определения конкретного ресурса, к которому требуется доступ
- Фильтрации: может использоваться в запросах для фильтрации списка ресурсов
- Ссылок: может использоваться в ссылках для указания на другие ресурсы

### 12. Что такое многоуровневая система
Это архитектурный принцип, который делит API на несколько уровней. Каждый уровень выполняет определенную функцию и взаимодействует только с соседними уровнями

Преимущества многоуровневой системы:
- Модульность - API становится более модульным и протым в разработке и обслуживании
- Масштабируемость - Каждый уровень можно мастабировать независимо, что позволяет оптимизировать API для разных типов нагрузки
- Повторное использование - Компоненты API можно повторно использовать на разных уровнях
- Безопасность - Разграничение уровней позволяет повысить безопасность API

Основные уровни REST API:
- Уроверь интерфейса - Определяет, как клиент взаимодействует с API
- Уровень приложения - Определяет бизнес-логику API
- Уровень доступа к данным - Определяет, как API взаимоействует с хранилищем данных

Пример:
1. Клиент - Веб-браузер или мобильное приложение
2. Уровень интерфейса - API Gateway, который принимает запросы от клиентов и маршрутизует их на соответствующие серверы
3. Уровень приложения - Сервер приложений, который обрабатывает запросы и бизнес-логику
4. Уровень доступа к данным - База данных, которая хранит данные

## END ---------------- REST vs SOAP ----------------

## HTTP
+ [1. Коды ответов](#1-коды-ответов)
+ [2. HTTP/2 vs HTTP/1.1](#2-http2-vs-http11)
+ [3. HTTP/2 vs HTTP/3](#3-http2-vs-http3)
+ [4. CSRF токен](#4-csrf-токен)

### 1. Коды ответов
- 1xx Информация - Содержит заголовки информирующие о процессе передачи. Это обыно предварительный ответ, состоящий только из Status-Line и опциональных заголовков, и завершается пустой строкой
- 2xx Успех - Указывает, что запрос клиента был успешно получен, понят и принят
- 3xx Перенаправление - Коды сообщают, что для успешного выполнения операции необходимо сделать друшлй запрос, как правило по другому URI
- 4xx Ошибка клиента - Указание ошибок со стороны клиента
- 5xx Ошибки сервера - Указывают на случаи, когда сервер знает, что произошла ошибка и не может обработать запрос

### 2. HTTP/2 vs HTTP/1.1
HTTP/1.1 Появилась в 1999 году. Это была первая стандартизированная версия
1. Появились методы:
- PUT - Заменяет все текущие представления сущности данными запроса
- DELETE - Удаляет указанную сущность
- OPTIONS - описывает параметры соединения с ресурсов
- TRACE - Проверяет обратную свять по пути к целевому ресурсу

Добавили позже
- CONNECT - устанавливает "туннель" к серверу
- PATCH - чстично заменяет указанную сущность даными из запроса

2. Заголовки
- Host - Позволяет указывать на домен, с которого пришел запрос
- keep-alive - 
  Чтобы передать данные по протоколу HTTP, надо установить TCP/IP соединение между клиентом и сервером. Она устанавливается перед каждый запросом,
  а после выполнения закрывается.</br>
  Эту проблему пытались решить в версии 1.0 с помощью keep-alive, которое позволяло создавать постоянные соединения</br>
  Для этого клиенту необходимо передать заголово `Connection: keep-alive`</br>
  В версии 1.1 keep-alive стал указываться по умолчанию</br>

3. Пересылка частями (chuncked)</br>
   Ввели механизм потокой передачи данных небольшми частями `Transfer-Encoding: chunked`

HTTP/2 появилась ы 2009 году. 
1. Бинарные данные - Данные передаются в бинарном формате. Бинарные данные меньше размером, поэтому скопрость передачи больше
2. Мультиплексирование - При передаче информация бьется на небольшие бинарные кусочки - фреймы. Они собираются в потоки, для каждого запрашиваемого ресурса создается отдельный.</br>
    Принимающая сторона самостоятельно соберет все фреймы из одного потока и обработает их
3. Сжатие заголовка HPACK - новый алгоритм сжатия. Он не использует старый DEFLATE и заточен под эффективную защиту от CRIME-атак
4. Приоритизация потоков - Позволяет управлять порядком обработки и передачи потоков данных на одном соединении. Улучшает производительность, делает управление ресурсами эффективнее</br>
    При отправке параллельных запросов на сервер клиент может расставить приоритеты запрашиваемых им ответов. Это делается присовением каждому потоку веса от 1 до 256
5. Server push - Позволяет серверу начать передачу ресурсов на клиент еще до запроса
   - Клиент отправляет запрос на сервер для загрузки страницы
   - Сервер анализирует и определяет, какие ресурсы будут нужны для полной загрузки страницы
   - Сервер создает множество потоков данных и начинает отправлять эти ресурсы через Server Push на клиент
   - Клиент получает ресурсы и может начать их загрузку

### 3. HTTP/2 vs HTTP/3
- HTTP/3 использует новый протокол передачи данных - QUIC (Quick UDP Internet Connections)
- В одном запросе объеденыны установка соединения и установление шифрования

### 4. CSRF токен
CSRF - Cross Site request Forgery</br>
Проблема: Браузеры не могут отличить было ли действие явно совершено или пользователь неумышленно выполнил действие</br>
Защитой от Csrf атак является токен. Под токеном имеется в виду сучайный набор байт, который сервер передает клиенту, а клиент возвращает серверу
</br>
Токен должен быть:
...
`JVM(Java Virtual Machine)` — это часть платформы Java, которая исполняет программы.</br>
программа, предназначенная для выполнения байт-кода. Преимущество - "Write once, run anywhere"

`JRE(Java Runtime Environment)` — это элемент Java, расположенный на диске, создающий и запускающий JVM</br>
это пакет всего необходимого для запуска скомпилированной Java программы

`JDK(Java Development Kit)` - позволяет разработчикам создавать Java-программы, которые могут выполняться и запускаться посредством JVM и JRE. JDK = JRE + компилятор</br>
...
+ [20. Финальный класс](#20-финальный-класс)
+ [21. Поверхностное копирование](#21-поверхностное-копирование)
+ [22. Глубокое копирование](#22-глубокое-копирование)
...
Обертка - это специальный класс, который хранит внутри себя значение примитива</br></br>
...
то просто возвращается ссылка, иначе создается новая строка в пуле, а затем возвращается ссылка</br></br>
...
Строка хранится в пуле строка до тех пор пока она не будет удалена сборщиком мусора</br></br>

Даже если мы думаем, что закончили работу с паролем, он остается доступен в памяти</br></br>
...
- Ее hashCode кэшируется в момент создания и нет необходимости расчитывать его снова. Это делает строку отличным кандидатом для ключа в Map</br>
...
Статическим классами могут быть вложенные классы</br></br>

Статические вложенные классы можно использовать вне своего родительского класса, ели у такого класса стоит модификатор доступа public</br></br>

Статические классы отличаются от обычных классов</br>
Методы вложенного статического класса имеют доступ ко всем статическим переменным и методам своего класса родителя, даже если те объявлены private</br></br>

Примеры обращения к статическим классам</br></br>
...
### 21. Поверхностное копирование
Поверхностное копирование позволяет копировать значения простого объекта в новый объкт, не включая значения объекта</br></br>
...
Возможность копировать значения составного объекта в другой новый</br>
...
- Если какой-либо слен класса не поддерживает клонирование, то в методе клонирования необходимо создать новый экземпляр этого класса и скопировать каждый его член со всеми атрибутами в новый объект класса по одному
...
### 3. Класс InputStream
Получает данные из различных источников</br>
- массив байтов
- строки
- файлы
- каналы pipe</br></br>
...
Определяет байтовый поток вывода</br>
...
Класс позволяет получить следующие данные о файле</br>
- права доступа
...
Для получения доступа из внутреннего класса к экземпляру его внешнего класса необходимо в ссылке указать имя класса и влючевое слово this</br>
OuterClass.this
...
Если конструктор у класса не задан, то по умолчанию создастся пустой конструктор без реализации, которые будет вызывать super</br></br>
...
Есть 2 типа загрузки классов</br>
- Статическая
- Динамическая
...
+ [7. Может ли метод main выбросить исключение во вне](#7-может-ли-метод-main-выбросить-исключение-во-вне)
...
1. Если в блоке try или catch есть System.exit
2. Возникнет смертельная ошибка (OutOfMemoryError)
3. В блоке try или catch произошел бесконечный цикл

### 7. Может ли метод main выбросить исключение во вне
...
```
1. Upper Bounded Wildcards - <? extends Animal></br>
...
2. Lower Bounded Wildcards - <? super Cat> - Можно положить:</br>
Это значит, что метод принимает на вход коллекцию объектов класса Cat либо любого другого класса предка Cat
Уго предок Pets и предок предка Animal
</br></br>
...
+ [29. Stack](#29-stack)
+ [30. Comparator vs Comparable](#22-глубокое-копирование)
...
- НАчало транзакции - Когда транзакцияначинается, PostreSQL устанавливает блокировку на уровне таблицы для всех таблиц, которые будут затронуты транзакцией
...
+ [8. Как реализовать оптимистическую блокировку](#7-что-такое-transactionmanager)
