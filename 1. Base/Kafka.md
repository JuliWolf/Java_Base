...
Соотношение партиций и консьюмеров в Kafka влияет на распределение нагрузки при обработке данных.

Если количество консьюмеров совпадает с количеством партиций в топике, то каждый консьюмер будет читать только из своей партиции и не будет видеть сообщения, которые находятся в других партициях. Это может быть полезно для обработки большого объёма данных и для масштабирования консьюмеров.

Если количество консьюмеров не совпадает с количеством партиций, то некоторые партиции будут обрабатываться несколькими консьюмерами. Это может быть полезно, если некоторые из партиций генерируют больше данных, чем другие.

При этом количество консьюмеров не должно быть больше, чем количество партиций. Число партиций определяет максимальный параллелизм, с которым можно обрабатывать данные.
...
+ [1. At-most-once delivery](#1-at-most-once-delivery)
+ [2. At-least-once delivery](#2-at-least-once-delivery)
+ [3. Exactly-once](#3-exactly-once)
...
В данной схеме дублирование обработки сообщений `A` может произойти, если после чтения сообщения случился отказ потребителя и не удалось выполнить коммит смещения. 
