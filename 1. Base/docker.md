...
+ [2. Сеть внутри кластера](#2-сеть-внутри-кластера)
+ [3. Обращение к хосту, созданному внутри кластера](#3-обращение-к-хосту-созданному-внутри-кластера)

#### 1. Локальная сеть
Обращение в localhost возможно при обращении одного контейнера к другому в рамках одного пода

#### 2. Сеть внутри кластера
Для создания одной сети внутри кластера надо использовать `ClusterIp` при создании сервера<br>
В таком случае будет создана env переменная со значением `{service_name}_service_host`

#### 3. Обращение к хосту, созданному внутри кластера
- `{service_name}_service_host` - создается env переменная
- "{service.name}.default" - значение будет автоматом подставлено
```yaml
env:
    - name: AUTH_ADDRESS
